...
var selection = 0,
    ...
    updateIdea = function (id, txt) {
      console.log('updating', id, txt);
      $.ajax({
        type: 'PUT',
        url:  '/idea/' + id,
        data: {txt: txt}
    ...
    updateDoc = debounce(function (id, doc) {
    ...
    domAddIdea = function (id, txt ,wc, published) {
      $("#ideaholder").prepend(['<a class="idea',
      (published) ? ' published' : '',
      '" id="idea',
      id,
      '" data-id="',
      ...
      id,
      '">',
      //(wc) ? '<span class="wc">'+wc+'</span>' : '',
      txt,      
      '</a>'].join(''));
    },
    publishIdea = function (id, val) {
      console.log('publishing', val, id);
      $.ajax({
        type: 'PUT',
        url:  '/idea/' + id,
        data: {published: val},
...
$("#newidea").bind('keyup blur', function (event) {
  if ((event.type === 'keyup' && event.which === 13 && $("#newidea").val() !== '')
      || (event.type === 'blur' && $("#newidea").val() !== '')) {
    addIdea($(this).val(), function (id, txt) {
      domAddIdea(id, txt, 0);
    });
    $("#newidea").val('');
  }
...
$("#delete").bind('click', function () {
    removeIdea($('#title').data('id'));
    window.location.hash = '#';
});

$("#ideaholder").delegate('input', 'keyup', function () {
  if (event.which === 13) {
    $("#newidea").focus();
  }
});

$("#title").bind('focusout blur', function () {
  updateIdea($(this).data('id'), $(this).val());
});

$("#textarea").bind('focusout blur change keypress keyup', function (event) {
  $("#writer").css('height',$("#writer").height());
  $(this).css('height','auto');
  $(this).css('height', event.target.scrollHeight + 40);  
  $("#writer").css('height','auto');
  updateDoc($(this).data('id'), $(this));
});

$("#publish").bind("click", function () {
  if ($(this).hasClass('published')) {
    //unpublish
    $(this).text('Publish').removeClass('published');
    $("#preview").text('Preview')
    publishIdea($('#title').data('id'), 0);
  } else {
    //publish
    $(this).text('Unpublish').addClass('published');
    $("#preview").text('View')
    publishIdea($('#title').data('id'), 1);
  }
});
...
$(window).bind('hashchange', function () {
  ...
  if (location.hash !== "" && location.hash !== "#") {
    ...
    $("#ideas").hide();
    $("#textarea, #title").data('id', id);
    $("#title").val("Loading...");
    $("#textarea").css('height', 82);
    $("#preview").attr('href', '/' + id)
    getDoc(id, function (body) {
      if (body.status === "good") {
        $("#title").val(body.txt)
        $("#writer").show();
        $("#textarea").val(body.doc);
        $("#textarea").trigger('change');
        if (body.published) {
          $("#publish").text('Unpublish').addClass('published');
          $("#preview").text('View')
        }
      } else {
        window.location.hash = '#';
    ...
    $("#writer").hide();
    $("#prelogin").hide();
    $("#ideaholder").html("Loading...");
    getIdeas(function (ideas) {
      var html,
          i = 0,
          l = ideas.length;
      $("#ideaholder").html("");
      for(i,l; i<l; i+=1) {
        domAddIdea(ideas[i].id, ideas[i].txt, ideas[i].wc || '', ideas[i].published);
      }
    });
  }
});
...
$('.browserid').bind('click',function() {  
    navigator.id.get(function gotAssertion(assertion) {
